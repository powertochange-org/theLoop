...
if ( !class_exists('adAuthenticationPlugin') ) {
	...
	class adAuthenticationPlugin {
		...
		function authenticate( $username, $password ) {
			...
			$multi_base_dns = explode("|", $base_dn);
			$ldap_base_dns_count = count($multi_base_dns);
			for ($jtb_i = 1; $jtb_i <= $ldap_base_dns_count; $jtb_i++) {
				$ldap_base_dn_id[] = $ldap;
				// _e(printf('-- ' . $jtb_i . ' --<br>', $this->ldap_auth_domain));
			}
			// _e(printf('multi_base_dns: ', $this->ldap_auth_domain));
			// _e(print_r($multi_base_dns), $this->ldap_auth_domain);
			// _e(printf('ldap_base_dn_count: ', $this->ldap_auth_domain));
			// _e(printf('"' . count($ldap_base_dn_id) . '"'));
			if ( !$ldap )
			...
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				$result = @ldap_search($ldap_base_dn_id, $multi_base_dns, $uid_filter, array('dn'));
			} else {
			...
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				...
				$ldap_user = @ldap_get_entries($ldap, $search);
			} else {
                                foreach ($result as $value) {
                                        if(ldap_count_entries($ldap,$value)>0){
                                                $search = $value;
...
$user_array = explode(',',$ldap_user);
$CN_array = explode('=',$user_array[0]);
$name_array = explode(' ',$CN_array[1]);
...
//						$last_name = $userinfo['sn'][0];
		...
		function _user_in_group( $ldap, $username, $group ) {
			...
			if ( !defined('WP_DEBUG') || ( defined('WP_DEBUG') && false === WP_DEBUG ) ) {
				$result = @ldap_search($ldap_base_dn_id, $multi_base_dns, $group_filter, array($group_attr));
			} else {
	...
	<form action="options.php" method="post">
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
					<input type="text" name="LDAP_authentication_base_dn" id="LDAP_authentication_base_dn" value="<?php echo $base_dn; ?>" size="100" /><br />
					<?php _e('Base DN (e.g., <code>ou=users,dc=example,dc=net</code>)<br />This should be the branch where you store users for Active Directory<br />DO NOT use the root DN. It will error out on the ldap search.<br />If you have Multiple DNs with users, list them all <code>seperated by pipe symbol "|"</code>', $this->ldap_auth_domain); ?><br />
				</td>
			</tr>
			<tr valign="top">
