...
function parseAdminInput(){
	...
	if (isAppAdmin('support_calculator_admin', 0)){
		$admin = mysql_real_escape_string($wpdb->dbh, htmlspecialchars($_GET["input_add_admin"]));
		...
		if (!$user){
			...
			$ID = $user->ID;
			$l = intval(get_user_meta($ID, 'loopadmin_support_calculator_admin', true) | pow(2, mysql_real_escape_string($wpdb->dbh, $_GET["input_admin_level"])));
...
function parseAdminRemove(){
	global $wpdb;
	if (isAppAdmin('support_calculator_admin', 0)){
		$admin = mysql_real_escape_string($wpdb->dbh, htmlspecialchars($_GET["input_remove_admin"]));
		$user = get_user_by('login', $admin );
		if ($admin==""){
			return;
		}
